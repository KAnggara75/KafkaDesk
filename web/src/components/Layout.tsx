...
const Layout: React.FC = () => {
	...
	return (
		<div className="bg-gray-50 dark:bg-slate-900 font-sans text-slate-900 dark:text-slate-100 overflow-x-hidden min-h-screen transition-colors duration-300">
			{/* Header */}
			<header className="h-12 bg-white dark:bg-dark border-b border-gray-200 dark:border-darklight flex items-center justify-between pl-2 pr-6 sticky top-0 z-50 transition-colors duration-300">
				<div className="flex items-center space-x-4">
					<button
						onClick={() => setIsSidebarOpen(!isSidebarOpen)}
						className="p-2 hover:bg-slate-50 dark:hover:bg-darklight rounded-md transition-colors text-slate-500 dark:text-slate-400"
					...
					<div className="w-48 select-none"> {/* Fixed width wrapper to prevent layout shift during transition */}
						<nav className="space-y-1">
							...
							{clusters.map((cluster) => {
								...
								return (
									<div key={cluster.name} className="space-y-1 border-t border-gray-100 dark:border-darklight">
										<button
											onClick={() => toggleCluster(cluster.name)}
											className="w-full flex items-center justify-between px-4 py-2 text-sm font-medium text-darklight dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-darklight transition-colors"
										>
										...
										{isExpanded && (
											<div className="pl-8 space-y-1 pb-2">
												...
												<Link className="block px-4 py-1.5 text-sm text-slate-500 dark:text-slate-400 hover:text-dark dark:hover:text-slate-200 hover:bg-slate-50 dark:hover:bg-darklight rounded transition-colors" to="#">Consumers</Link>
												<Link className="block px-4 py-1.5 text-sm text-slate-500 dark:text-slate-400 hover:text-dark dark:hover:text-slate-200 hover:bg-slate-50 dark:hover:bg-darklight rounded transition-colors" to="#">ACL</Link>
											</div>
										)}
									</div>
